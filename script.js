        ...
        .then((snapshot) => {
          ...
          db.ref(`categories/${currentCategory}/threads/${currentPostId}/replies`).push({
            ...
            if (post.userId && post.userId !== user.uid) {
              ...
              // Add notification directly to the user's notifications collection
              db.ref(`users/${post.userId}/notifications`).push(notificationData)
                .then(() => console.log("Reply notification sent"))
...
function loadNotifications() {
  ...
  db.ref(`users/${auth.currentUser.uid}/notifications`)
    ...
    .on('value', (snapshot) => {
      ...
      // For debugging: log the notifications to console
...
// Add this helper function to format timestamps in a human-readable way
function formatTimestamp(timestamp) {
...
// Improved function to navigate to the right source
function navigateToSource(type, sourceId) {
  if (!sourceId) return;
  
  console.log(`Navigating to ${type} notification with sourceId: ${sourceId}`);
  
  // For both reply and upvote notifications, find the relevant thread
  db.ref('categories').once('value', (snapshot) => {
    let found = false;
    ...
    snapshot.forEach((categorySnapshot) => {
      ...
      threadsRef.forEach((threadSnapshot) => {
        ...
        // Direct match with thread ID (post notifications)
        if (threadKey === sourceId) {
          console.log(`Found post in category ${categoryKey}, opening...`);
          openPost(categoryKey, threadKey, threadSnapshot.val());
          found = true;
          return true;
        }
        
        // Check replies within this thread
        if (!found && threadSnapshot.hasChild('replies')) {
          const repliesRef = threadSnapshot.child('replies');
          
          repliesRef.forEach((replySnapshot) => {
            if (replySnapshot.key === sourceId) {
              console.log(`Found reply in post ${threadKey} in category ${categoryKey}, opening post...`);
              openPost(categoryKey, threadKey, threadSnapshot.val());
              found = true;
              return true;
            }
    ...
    if (!found) {
      console.warn(`Could not find source with ID: ${sourceId}`);
    }
  });
}
...
// Ensure the notification bell is properly initialized
document.addEventListener('DOMContentLoaded', () => {
  // ...existing code...
